...
def calcScaleZeroPoint(min_val, max_val, num_bits=8):
    ...
    zero_point = qmax - max_val / scale   #量化后的zero-point
...
def quantize_tensor(x, scale, zero_point, num_bits=8, signed=False):
    ...
    q_x.clamp_(qmin, qmax).round_()   #clamp 超过最大值最小值的部分使用qmin和qmax
...
class QConv2d(QModule):
    ...
    def forward(self, x):
        if hasattr(self, 'qi'):
            self.qi.update(x)   #更新 x 的 min，max，score，
            x = FakeQuantize.apply(x, self.qi)

        self.qw.update(self.conv_module.weight.data)  #更新 qw 的 min ，max，score
